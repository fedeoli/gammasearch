...
//					normalization factor
				...
				G_smoothed = (uchar)G;
				temp_image->imageData[i*output->widthStep + j] = (uchar) G_smoothed;
			}
...
//	VERTICAL SMOOTHING
			for (i = 0; i < input->height ; i++)
				for (j = 0; j < input->widthStep ; j++) {
					...
					G_smoothed = (uchar) G;
					output->imageData[i*output->widthStep + j] = (uchar)G_smoothed;
				}
...
//	kernel allocation
	float* kernel = (float*)malloc((2 * k + 1) * (2 *k + 1)*sizeof(float));

//	kernel assignement
		for(u=0;u<(2*k+1);u++)
			for(v=0;v<(2*k+1);v++)
				kernel[u*(2*k+1)+v]= exp(-(pow(u - k, 2) + pow(v - k, 2)) / (2 * pow(sigma, 2))) / (2 * M_PI* pow(sigma, 2));
				...
				if (((i - k) >= 0) && ((j - k) >= 0) && ((i + k) <= input->height) && ((j + k) <= input->widthStep))
					...
					for (u = 0; u < (2 * k + 1); u++)
						for (v = 0; v < (2 * k + 1); v++) {
							temp = ((uchar*) input->imageData)[(i + u - k)* input->widthStep + (j + v - k)];
							datavalue = (int) temp;
							G = G + datavalue * kernel[u * (2 * k + 1) + v];
						}
				else {
...
//					contour normalization factor
					n_f = 0;
					for (u = i_start; u < i_end; u++)
						for (v = j_start; v < j_end; v++) {
							n_f = n_f + kernel[u * (2 * k + 1) + v];
						}
					...
					for (u = i_start; u < i_end; u++)
						for (v = j_start; v < j_end; v++) {
							temp = ((uchar*) input->imageData)[(i + u - k)* input->widthStep + (j + v - k)];
							datavalue = (int) temp;
							G = G + datavalue * kernel[u * (2 * k + 1) + v] / n_f;
						}
				}
				G_smoothed = (int)G;
				output->imageData[i*input->widthStep + j] = (uchar)G_smoothed;
			}
...
void median_filter(IplImage *input, IplImage *output) {
	...
	for(i=0; i<input->height; i++)
		for (j = 0; j < input->width; j++) {
			...
			output->imageData[i*input->width + j] = (uchar)ord[4];
		}
}
...
void median_cross_filter(IplImage *input, IplImage *output) {
	...
	for (i = 0; i<input->height; i++)
		for (j = 0; j < input->width; j++) {
			...
			output->imageData[i*input->width + j] = (uchar)ord[2];
		}
}
