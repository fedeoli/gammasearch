				...
				for d in range(bin_treshold_min, bin_treshold_max+1):
					for e in range(int(10*intensity_treshold_min), int(10*intensity_treshold_max)+1):
						for f in range(baricenter_dist_min, baricenter_dist_max+1):
							...
							tmp_error, tmp_mean = call_data_analysis(logfile, fits_folder, center_type, str((a/10)), str((b/100)), str(c), str(d), str((e/10)), str(f))
							print(str(tmp_error) + '\t' + str(error) + '\t' + str(a/10) + '\t' + str(b/100) + '\t' + str(c) + '\t' + str(d) + '\t' + str(e/10) + '\t' + str(f) )
