...
############################################################
	...
	# sources, whose number is limited to 1. The background level is also generated. After that, the 
	# commands "ctobssim" and "ctbin" will be invoked and an image file "cube.fits" for each map will be 
	# created and then stored in a dedicated folder. The files that will be stored in the those 
	# directories are:
	...
	for i in tqdm(range(0, n_maps)): 
		...
		log.write('\n' + filename + '\t\t')
		background_prefactor = map_creator.background_generation(map_creator.min_background, \
															    sigma**2, log)
		source_array = map_creator.source_generation_INAF(coordinates, background_prefactor, \
														sigma, log)
		map_creator.sourcefile_generator(source_array, background_prefactor,  filename)
		os.system("mv " + filename + " Generated_Maps/" + filename)
	
		os.system( 'ctobssim inmodel=Generated_Maps/' + filename + ' ra=' + str(coordinates[0]) + \
				   ' dec=' + str(coordinates[1]) +  ' rad=10 tmin=2020-01-01T00:00:00 \
				   tmax=2020-01-01T00:15:00 emin=0.1 emax=100.0 caldb=prod2 irf=South_0.5h \
				   outevents=events.fits seed=' + str(i) )
		os.system( 'ctbin inobs=events.fits coordsys=CEL proj=CAR xref=' + str(coordinates[0]) + \
				   ' yref=' + str(coordinates[1])  + ' binsz=0.02 nxpix=200 nypix=200 ebinalg=LOG \
				   emin=0.1 emax=100 enumbins=1 outcube=cube.fits'  )
...
def usage():
	...
	parser.add_argument('DEC source coordinate', metavar = 'DEC', type=str, help='RA source \
						coordinate - celestial coordinates' )
	parser.add_argument('source significativity', metavar = 'sigma', type=float, help='ratio between \
						source intensity and the square root of the background intensity' )
	parser.add_argument('number of generated maps', metavar = 'n_maps', type=int, help='number \
						of generated maps' )
