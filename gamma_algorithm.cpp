...
//	IplImage vars
	IplImage* source_image, * elab_image[10], * elab_image_color[5], *back;

//	fitsfile vars
	fitsfile * fits_pointer, *prova, *out, *open, *fits_dest, *background;
...
//	other vars
	int status =0, bitpix =0, dim =0, hdu_type =0, hdu_num=0, hdu_nums=0, *L, n_sources, *baricenter_int,
		 *area, *values, radius, * max_int, binarization_treshold, center_distance, *rescale_array, i, j, temp, datavalue;
...
//	creation of a IplImage with the same size of the fits image, retrieved in the previous read_param function.
	...
	back = cvCreateImage(gamma_image, IPL_DEPTH_8U, 1);
	elab_image[6] = cvCreateImage(gamma_image, IPL_DEPTH_8U, 1);
	elab_image[7] = cvCreateImage(gamma_image, IPL_DEPTH_8U, 1);
	elab_image[8] = cvCreateImage(gamma_image, IPL_DEPTH_8U, 1);
...
//	median filtering
	//median_cross_filter(source_image, elab_image[7]);
	//median_filter(source_image, elab_image[7]);
	mean_cross_filter(source_image, elab_image[7]);

//	Image display if visual == "y"
	if(*visual == 'y')
			cvShowImage("median filter", elab_image[7]);
...
//	gauss_smooth_fast(elab_image[6], elab_image[0], sigma_spa);
	gauss_smooth_fast(elab_image[7], elab_image[0], sigma_spa);
...
//	Image display if visual == "y"
		if(*visual == 'y')
			cvShowImage("binary image", elab_image[1]);
