...
def true_coordinates(logfile, n_maps):
	...
	# vars initialisation
	coord_array = [] 										# array with the sources coordinates
...
def convert_data(n_maps, center_type, sigma_spa, accept_level, radius, binary_treshold, intensity_treshold, baricenter_distance, fits_dir):
	...
	name, junk = order.directory_order(fits_dir)
	...
	for i in range(0,n_maps):
		...
		call(["./analyze_map_smooth.bin", fits_dir + "/"+ str(name[i]), center_type, "logfile_map", "n", "n", sigma_spa, accept_level, radius, binary_treshold, intensity_treshold, baricenter_distance])
